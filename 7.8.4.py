...
class Queue(Collection):
    ...
    @add.register(list)
    def _from_list(self, pairs :list):
    ...
    @add.register(dict)
    def _from_dict(self, pairs :dict):
    ...
    def __str__(self):
        return f"Queue({[item for item in self.pairs]})"
    ...
    def del_all(self,value):
        while self.check(value):
            curent_queue = [item[0] for item in self.pairs]
            index = curent_queue.index(value[0])
            del self.pairs[index] 
            
    def check (self,value):
        curent_queue = [item[0] for item in self.pairs]
        if value[0] in curent_queue:
            return True
        return False    
'''       
queue = Queue([('Allison Finley', 14), ('Bryan Bradshaw', 48)])
print(queue)
# chain = Chain ('Allison Finley', 14)
queue.add(tuple(('Allison Finley', 14)))

# print(chain in queue)
